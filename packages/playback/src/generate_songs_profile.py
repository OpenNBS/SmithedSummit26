    ...
    *,
    ...
    for song_data in song_manifest:
        ...
        _append(
            ...
            f"songs.{region} append value "
            f"{_song_entry_snbt(song_id, song_index, formatted_string, title, author)}",
        )
        ...
        for chord in get_notes(song):
            ...
            root_commands: list[str] = [
                ...
                f"data modify storage nbs:temp input.song set value {_snbt_string(song_id)}",
                f"data modify storage nbs:temp input.tick set value {tick}",
                f"execute store result score {region}.#len nbs "
                f"if data storage nbs:playback locations.{region}[]",
                f"execute store result storage nbs:temp input.i int 1 "
                f"run scoreboard players set {region}.#iter nbs 0",
                f"function nbs:playback/{region}/speaker_iter/root with storage nbs:temp input",
                f"scoreboard players add notes_played nbs_stats {len(notes)}",
                "scoreboard players add ticks_played nbs_stats 1",
            ]
            _define(functions, f"nbs:song/{song_id}/{tick}/root", root_commands)
            ...
            for note in notes:
                for speaker in speaker_ranges:
                    ...
                    speaker_fn = f"nbs:song/{song_id}/{tick}/{speaker_type}"

                    if note.instrument == "BEAT":
                        beat_commands = [
                            f"execute store result score #random nbs run random value 1..{anim_count}",
                        ]
                        for i in range(anim_count):
                            beat_commands.append(
                                f"execute if score #random nbs matches {i + 1} "
                                f"if entity @a[distance=0..{outer_range}] "
                                f"run function summit_ambiance:speaker_{region}/"
        ...
        _define(
            functions,
            f"nbs:song/{song_id}/{tick + 40}/root",
            [f"function nbs:playback/{region}/advance"],
        )
